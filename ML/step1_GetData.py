            ...
            while now_page <= pageCnt :
                ...
                if res2.status_code == requests.codes.ok :
                    ...
                    if chk == False : # 첫 번째 영화이면
                        ...
                        df.to_csv('./ML/naver_data.csv', index = False, encoding = 'UTF-8')
                        chk = True
                    else : # 두 번째 영화부터
                        df.to_csv('./ML/naver_data.csv', index = False, encoding = 'UTF-8', mode = 'a', header = False)
